...
meta = {}           # 각 추적 ID별 메타데이터(처음 본 시각, 마지막으로 본 프레임, 연속 미검출 횟수) # {id: {"first_seen":ts, "last_seen_frame":n, "missed":k}}
...
while True:
    ...
    if frame_count % detect_interval == 0: 
        ...
        for box in results.boxes: # 각 객체의 경계박스 좌표와 클래스, 신뢰도 추출
            ...
            cls = int(box.cls[0])
            if cls == 0 and score >= conf_threshold:  # 사람(클래스 0)만 필터링
                detections.append([x1, y1, x2, y2, score]) # detections는 [x1,y1,x2,y2,score] 형태의 리스트를 누적

        detections = np.array(detections, dtype=np.float32) if detections else np.empty((0, 5), dtype=np.float32) # ByteTrack 입력 형식에 맞게 NumPy 배열 변환

        # ByteTrack으로 추적 업데이트 (새로운/기존 트랙 갱신)
        online_targets = tracker.update(detections, [h, w], [h, w])
    else:
        online_targets = [] # 감지 주기가 아닐 경우 빈 리스트로 유지
    ...
    if target_id is None and tracks: # 타겟이 없다면 중심 x 좌표가 가장 작은(왼쪽) 사람을 선택하여 target_id로 고정
        leftmost = min(tracks, key=lambda t: center_x(t[:4]))
